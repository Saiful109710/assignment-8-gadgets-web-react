...
const Cart = () => {
  ...
  return (
    <div>
        <div className='flex justify-between items-center my-5 container mx-auto p-3'>
            <div>
                <h2 className='text-xl font-bold'>Cart</h2>
            </div>
            <div className='flex flex-col md:flex-row gap-4 items-center'>
                <h2 className='text-lg font-bold'>Total Cost: {totalPrice}</h2>
                <button onClick={()=>handleSort(products)} className='btn bg-transparent border-purple-700 text-purple-700 md:px-8 rounded-3xl'>Sort By Price</button>
                <button disabled={totalPrice===0} className='btn bg-purple-700 text-white md:px-8 rounded-3xl' onClick={()=>handlePurchase(totalPrice)}>Purchase</button>
                {
                    isModalOpen && (
